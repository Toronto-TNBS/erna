...
def main():
    ...
    select_channel = channels_all.index(st.selectbox('Enter channel to import: ', channels_all))
    ...
    hfs = int(sidebar_filename[-8:-6])
    
    if hfs == 0:
        hfs = 100
    
    peak_locs = sp.signal.find_peaks(raw_data, height=4, distance=round((1/hfs)/2*fs))[0]
    
    win = int(np.floor((1/hfs)*fs))
    
    evoked_fields = np.empty([1, win])
     
    x = st.slider('Adjust Window',min_value=-50,max_value=50, value = 0)
    ...
    auto_win = st.slider('Adjust Auto-calulation window:', min_value=1, max_value=20, value = 2)
    ...
    #data = np.concatenate(np.array(baseline_list_t), np.array(peaks_list_t),np.array(troughs_list_t))
    ...
    press = st.sidebar.button('Save Data!')
    
    if press == True:
        df.to_excel(sidebar_filename[0:-4]+'.xlsx', index=False)
        fig3.write_html(sidebar_path+"/"+sidebar_filename[0:-4]+".html")
...
sidebar_path = st.text_input('Path to .smr files','/Users/srdjan/Desktop/files for peak modulation smoothed')
